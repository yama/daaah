...
if( $modx->db->getRecordCount( $result ) >= 1 ) {
	while( $row = $modx->db->getRow( $result ) ) {
		...
		$s_drop_down_history .= '>';
		$s_drop_down_history .= iconv('CP932', 'UTF-8', strftime(iconv('UTF-8', 'CP932', '%Y年%m月%d日(%a)%H時%M分%S秒' ) , $row['editedon'] ));
...
<div class="tab-pane" id="daaahPane">
	...
	<div class="tab-page" id="tabGeneral">
		...
		<span class="warning"><?php echo iconv('CP932', 'UTF-8', strftime(iconv('UTF-8', 'CP932', '%Y年%m月%d日(%a)%H時%M分%S秒' ) , $hisid )) ?></span>に承認を受けた内容と<span class="warning">現在、編集中のページデータ</span>との差分を表示しています。<br />　
	...
	<div class="tab-page" id="tabPreview">
		...
		<table width="96%" border="0"><tr><td>ここには<?php echo iconv('CP932', 'UTF-8', strftime(iconv('UTF-8', 'CP932', '%Y年%m月%d日(%a)%H時%M分%S秒' ) , $hisid )) ?>に承認を受けた内容をプレビューしています。</td></tr>
